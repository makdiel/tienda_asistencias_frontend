...
type TabType = 'dashboard' | 'empleados' | 'asistencias' | 'scanner'| 'reporte';
...
export default function AdminHome() {
  ...
  // Estados para tienda  useState<Tienda[]>([]);
  ...
  const [fechaInicio, setFechaInicio] =useState<string>(""); // ahora string
  const [fechaFin, setFechaFin] = useState<string>(""); // ahora string
  const [horaMaxima, setHoraMaxima] = useState('08:00'); // ej: 8 AM
  const [horaSalidaMax, setHoraSalidaMax] = useState('17:00'); // ej: 5 PM
 ...
 const formatDate = (date: Date): string => {
  ...
  try {
    const res = await api.get(`/reportes/reporte-asistencia?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}&idTienda=${tiendaSeleccionada}&horaMaxima=${horaMaxima}&horaSalidaMax=${horaSalidaMax}`);
    const data = Array.isArray(res.data) ? res.data : [];
    setReporte(data);
  } catch (err) {
    console.error(err);
...
{/*}
    ...
    >
      ...
      <View style={styles.dateTimeCard}>
       {/*} <Text style={styles.currentDate}>{formatDate(currentDateTime)}</Text> */}
        <Text style={styles.currentDate}>{formatDate2(currentDateTime)}</Text>
  ...
  <ScrollView style={{ padding: 16 }}>
    <Text style={styles.cardTitle}>Reporte de Asistencia</Text>
  <View>
       
      {/* Fecha Inicio */}
      <TouchableOpacity onPress={() => setShowInicio(true)} style={styles.input2}>
        <Text style={{ color: fechaInicio ? "#000" : "#999" }}>
      ...
      {showInicio && (
        <DateTimePicker
          ...
          onChange={(event, selectedDate) => {
            setShowInicio(false);
            if (selectedDate) setFechaInicio(formatDate2(selectedDate));
          }}
        />
      )}

      {/* Fecha Fin */}
      <TouchableOpacity onPress={() => setShowFin(true)} style={styles.input2}>
        <Text style={{ color: fechaFin ? "#000" : "#999" }}>
      ...
      {showFin && (
        <DateTimePicker
          ...
          onChange={(event, selectedDate) => {
            setShowFin(false);
            if (selectedDate) setFechaFin(formatDate2(selectedDate));
          }}
        />
      )}
    ...
    <TextInput
      style={styles.input2}
      placeholder="Hora máxima entrada (HH:MM)"
      value={horaMaxima}
      onChangeText={setHoraMaxima}
    />
    <TextInput
      style={styles.input2}
      placeholder="Hora máxima salida (HH:MM)"
      value={horaSalidaMax}
      onChangeText={setHoraSalidaMax}
    />
  </View>
    <Picker style={styles.input2}
      selectedValue={tiendaSeleccionada}
      onValueChange={(itemValue) => setTiendaSeleccionada(itemValue)}
    >
      <Picker.Item label="Seleccionar tienda" value={tienda?.id} />
      {/* {tienda.map((item,index) => (<Picker.Item key={index} label={item.nombre} value={item.id} /> ))}   */}
      <Picker.Item key={user?.id_tienda} label={tienda?.nombre} value={user?.id_tienda} />
    </Picker>

    <TouchableOpacity style={styles.saveButton} onPress={fetchReporte}>
      <Text style={styles.saveButtonText}>Generar Reporte</Text>
    </TouchableOpacity>

    <View style={[styles.rowCard, styles.cardText]}>
    <Text style={[styles.rowText, styles.cardText]}>Emp</Text>
    <Text style={[styles.rowText, styles.cardText]}>Tienda</Text>
    <Text style={[styles.rowText, styles.cardText]}>Dias</Text>
    <Text style={[styles.rowText, styles.cardText]}>Tardes</Text>
    <Text style={[styles.rowText, styles.cardText]}>Extras</Text>
  </View>

    {/* Resultados */}
    {reporte.map((item, index) => (
      <View key={index} style={styles.rowCard}>
      <Text style={styles.rowText}>{item.NombreEmpleado}</Text>
      <Text style={styles.rowText}>{item.NombreTienda}</Text>
      <Text style={styles.rowText}>{item.DiasAsistidos}</Text>
      <Text style={styles.rowText}>{item.DiasTarde}</Text>
      <Text style={styles.rowText}>{item.HorasExtras}</Text>
    </View>
    ))}
  </ScrollView>
);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <Text style={[styles.tabText, activeTab === 'reporte' && styles.activeTabText]}>
            Reportes
          </Text>
        </TouchableOpacity>
      </View>
